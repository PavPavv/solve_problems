...
class TreeConvertor {
    ...
    private _makeTreeReduced(array: MyNode[]): TreeCode[]  {
        return array.reduce((tree: TreeCode[], node: MyNode) => {
            const newTree = new TreeCode(node.id, node.text, []);
            if (node.children.length) {
                newTree.children = this._makeTreeReduced(node.children);
            }
            tree.push(newTree);
            return tree;
        }, []);
    ...
    public setTreeReduced(): void {
        this._tree = this._makeTreeReduced(this._data);
    }
